...
class SubtaskSideviewFeatures:
    def __init__(self, proj_dir, output_dir, camera_pair='CAM_LL_CAM_LR', num_videos_to_process_per_camera=50):
        self.proj_dir = proj_dir
        self.output_dir = output_dir
        self.camera_pair = camera_pair.split('_')
        self.num_videos_to_process_per_camera = num_videos_to_process_per_camera

    def extract_landmarks(self):
        subtask = os.path.basename(self.proj_dir)
        os.makedirs(os.path.join(self.output_dir, subtask), exist_ok=True)

        # Create a future list for concurrent processing
        futures = []
        with ProcessPoolExecutor(max_workers=2) as executor:
            for camera in self.camera_pair:
                camera_files = [
                    file for file in os.listdir(self.proj_dir)
                    if file.endswith('.mp4') and camera in file
                ]
                tasks = self.prepare_tasks(camera_files, subtask)
                if tasks:
                    future = executor.submit(self.process_tasks, tasks)
                    futures.append(future)
                else:
                    logging.info(f'No new videos to process for camera {camera}.')

            # Wait for all futures to complete
            for future in as_completed(futures):
                logging.info(f'Tasks completed: {future.result()}')

    def prepare_tasks(self, files, subtask):
        tasks = []
        processed_count = 0
        for file in files:
            ...
            if not os.path.exists(output_hdf5):
                ...
                processed_count += 1
                if processed_count >= self.num_videos_to_process_per_camera:
                    break
        return tasks
    ...
    def process_tasks(self, tasks):
        ...
        return f'Processed {len(tasks)} videos.'
