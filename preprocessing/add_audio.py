...
class AddAudioData:
    ...
    def __init__(self, project_dir: str, start_participant: int = 1, end_participant: int = 10,
        ...
        self.participant_dir = sorted([os.path.join(self.project_dir, f'p{participant_id:02d}') for participant_id in
        ...
        self.logger = setup_calibration_video_logger(
            ...
            error_log_file='audio-video_log.txt',
    ...
    def add_max_audio(self):
        ...
        for participant_dir in self.participant_dir:
            ...
            part_video_count = 0
            print(f'Current Camera Directory: {current_camera_directory}')
            for video_file in os.listdir(current_camera_directory):
                print(f'Video File: {video_file}')
                if video_file.endswith(('mp4', 'avi')):
                    print(f'Yes Video exists')
                    ...
                    self.logger.debug(f'Audio added to {current_participant}/CAM_AV_P/{video_file} saved into {output_video_path}')
                    part_video_count += 1
            self.logger.info(f'Audio added to {part_video_count} videos in {os.path.basename(participant_dir)}')
    ...
    def get_video_bitrate(self, video_path):
        ...
        try:
            ...
            self.logger.info(f'BitRate of {os.path.basename(video_path)}: {int(bitrate)/1000} kbps')
            return bitrate
        except subprocess.CalledProcessError as e:
            self.logger.warning(f'Failed to get bitrate for video {video_path}: {str(e)}')
            return None
    ...
    def add_audio_var_video(self, video_path: str = None, output_video_path: str = None,
        ...
        self.logger.info(f'Video successfully written to {output_video_path}')
        
        return output_video_path


if __name__ == '__main__':
    project_path = '/media/BlueHDD/QUB-PHEO-datasets'
    # project_path = '/home/exponential_r/Documents/QUB-PHEO'
    Project = AddAudioData(project_path, start_participant=42, end_participant=64)
