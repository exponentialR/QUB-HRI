...
def update(frame_idx):
    ...
    ax.scatter(face[:, 0], face[:, 1], c='red', label='Face Landmarks')

    # Plot left hand landmarks
    left_hand = left_hand_landmarks[frame_idx]
    ax.scatter(left_hand[:, 0], left_hand[:, 1], c='blue', label='Left Hand Landmarks')

    # Plot right hand landmarks
    right_hand = right_hand_landmarks[frame_idx]
    ax.scatter(right_hand[:, 0], right_hand[:, 1], c='green', label='Right Hand Landmarks')

    # Plot pose landmarks
    pose = pose_landmarks[frame_idx]
    ax.scatter(pose[:, 0], pose[:, 1], c='purple', label='Pose Landmarks')
...
ani.save('left_landmark_video.mp4', writer='ffmpeg', fps=30)
