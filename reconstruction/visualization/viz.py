...
left_hdf5_file = '/home/iamshri/Documents/Dataset/p01/CAM_LL/landmarks/BIAH_BS.hdf5'
right_hdf5_file = '/home/iamshri/Documents/Dataset/p01/CAM_LR/landmarks/BIAH_BS.hdf5'
...
#         self.face_landmarks_left, self.left_hand_landmarks_left, self.pose_landmarks_left, self.right_hand_landmarks_left = self.load_keypoints(left_hdf5_file)
#
#         # Load keypoint data for the right view
#         self.face_landmarks_right, self.left_hand_landmarks_right, self.pose_landmarks_right, self.right_hand_landmarks_right = self.load_keypoints(right_hdf5_file)
#
#         # Projection matrices
#         self.P1 = np.hstack((self.camera_matrix_left, np.zeros((3, 1))))
#         self.P2 = np.hstack((self.camera_matrix_right, self.T.reshape(3, 1)))
#
...
#         fig.update_layout(
#             scene=dict(
#                 xaxis=dict(title='X', range=[np.min(points_3d[:, 0]), np.max(points_3d[:, 0])]),
#                 yaxis=dict(title='Y', range=[np.min(points_3d[:, 1]), np.max(points_3d[:, 1])]),
#                 zaxis=dict(title='Z', range=[np.min(points_3d[:, 2]), np.max(points_3d[:, 2])]),
#                 bgcolor="rgb(240, 240, 240)",
#                 xaxis_showgrid=True,
#                 yaxis_showgrid=True,
#                 zaxis_showgrid=True,
#             ),
#             title='3D Keypoints Visualization',
#             width=800,
#             height=800,
#         )
#
...
# frame_number = 304  # Frame number to visualize
# points_3d = stereo_reconstruction.get_3d_points_for_frame(frame_number)
#
# # Visualize the 3D points using Plotly
...
class Stereo3DReconstruction:
    ...
    def visualize_3d_points(self, points_3d):
        ...
        sc = ax.scatter(points_3d[:, 0], points_3d[:, 1], points_3d[:, 2], c=points_3d[:, 2], cmap='viridis', marker='o')
...
# Instantiate the class
# stereo_visualization = Stereo2DVisualization(left_hdf5_file, right_hdf5_file)

# Visualize 2D points for frame 303
# frame_number = 303  # Frame number to visualize
# stereo_visualization.visualize_2d_points(frame_number)
